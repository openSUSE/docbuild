...
class Doctype(BaseModel):
    ...
    * ``LANGS``: zero or more languages, separated by comma.
    ...
    def __contains__(self: Self, other: "Doctype") -> bool:
        ...
        return all(
            [
                self.product == other.product or self.product.value == "*",
    ...
    def iter_doctypes(self: Self,
        ...
        """
        has_product_wildcard = self.product.value == "*"
                ...
                docsets = sorted(
                    {
                        ...
                        for docset in portal_root.xpath(
                            f"product[@id={product_value.value!r}]/docset/@path",
                        )
                        if docset
                    },
                )
                    ...
                    for lang in sorted(
                        {
                            ...
                            for lang in portal_root.xpath(
                                (
                                    f"product[@id={product_value.value!r}]"
        ...
        for product_value, docset, lang in sorted(
            combinations,
            key=lambda item: (item[0].value, item[1], item[2].language),
        ):
