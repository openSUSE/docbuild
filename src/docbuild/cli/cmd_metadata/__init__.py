...
async def process_deliverable(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        dir=str(temp_repo_dir), prefix=f'clone-{prefix}_',
        delete=False
    ) as worktree_dir_str:
...
async def process_doctype(
    ...
    temp_repo_dir = (
        context.envconfig.get('paths', {}).get('temp_repo_dir', None)
    )
    ...
    tasks = [process_deliverable(deliverable,
                                 repo_dir=repo_dir,
...
@click.pass_context
def metadata(
    ctx: click.Context, doctypes: tuple[Doctype],
) -> None:
