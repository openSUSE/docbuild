...
from docbuild.tasks import metadata as metaprocess_pkg
from docbuild.tasks.metadata import (
    collect_files_flat,
    generate_metadata as process,
...
def mock_envconfig(tmp_path: Path) -> Mock:
    ...
    mock_paths.meta_cache_dir.mkdir(parents=True, exist_ok=True)

    mock_paths.tmp_repo_dir = tmp_path / "tmp_repos"
    mock_paths.main_portal_config = tmp_path / "config" / "dummy.xml"

    mock_tmp = Mock()
    mock_tmp.tmp_metadata_dir = tmp_path / "tmp" / "metadata"
    mock_paths.tmp = mock_tmp
...
def stitchnode(deliverable: Deliverable) -> etree._ElementTree:
    """Build a minimal stitched `docservconfig` ElementTree."""
...
class TestProcessDoctype:
    ...
    async def test_success_with_deliverables(
        ...
        env = mock_envconfig.paths
        result = await process_doctype(
            mock_root,
            doctype,
            repo_dir=env.repo_dir,
            tmp_repo_dir=env.tmp_repo_dir,
            meta_cache_dir=env.meta_cache_dir,
            dapsmetatmpl=mock_envconfig.build.daps.meta,
            exitfirst=False
        )
    ...
    async def test_no_deliverables_found(
        ...
        env = mock_envconfig.paths
        result = await process_doctype(
            mock_root, doctype, env.repo_dir, env.tmp_repo_dir, env.meta_cache_dir, mock_envconfig.build.daps.meta
        )
    ...
    async def test_exitfirst_fails_fast(
        ...
        failed = await process_doctype(
            mock_root, doctype, env.repo_dir, env.tmp_repo_dir, env.meta_cache_dir, mock_envconfig.build.daps.meta, exitfirst=True
        )
...
class TestProcessEmptyDoctypes:
    ...
    async def test_process_empty_doctypes(
        ...
        env = mock_envconfig.paths
        await process(
            env.main_portal_config, env.tmp.tmp_metadata_dir, env.repo_dir,
            env.tmp_repo_dir, env.meta_cache_dir, mock_envconfig.build.daps.meta, ()
        )
...
def test_store_productdocset_json_merges_and_writes(
    ...
    meta_cache_dir = mock_envconfig.paths.meta_cache_dir
    ...
    with patch.object(
        ...
        store_productdocset_json([doctype], stitchnode, meta_cache_dir)
...
class TestUpdateRepositories:
    ...
    async def test_update_repositories_success(
        ...
        from docbuild.tasks.metadata import update_repositories

        await update_repositories(deliverables, repo_dir) # type: ignore[arg-type]
